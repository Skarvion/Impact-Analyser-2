...
//##ObjectMastery - Start Changes
type
  TCustomReportMemoryLeaksOnShutdownProc = function(var ABuffer: array of AnsiChar; ACount: Cardinal): Integer; // returns new count of ABuffer
var
  DoLeakLogging: Boolean = True;
  CustomReportMemoryLeaksOnShutdownProc: TCustomReportMemoryLeaksOnShutdownProc = nil;
  ShowLeakMessages: Boolean = False;
//##ObjectMastery - End Changes
...
//##ObjectMastery - Start Changes
      if (Assigned(CustomReportMemoryLeaksOnShutdownProc)) then begin
        CustomReportMemoryLeaksOnShutdownProc(LLeakMessage, Cardinal(LMsgPtr) - Cardinal(@LLeakMessage[1]));
      end;

      if (ShowLeakMessages or (DelphiIsRunning and (DebugHook <> 0))) then begin
        {$ifndef NoMessageBoxes}
          {Show the message}
